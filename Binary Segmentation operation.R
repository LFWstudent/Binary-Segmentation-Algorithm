...
#' (k_1, k_2).
#' 
#' Accepts: data(observations), k_1(lower open interval bound), k_2(upper open interval bound)
#' 
#' Returns: test.stat (vector containing all CUSUM values for all points in the interval)
...
#' assuming the error is standard normal, we will use resampling. We take the 99%-ile of the simulated CUSUMs
#' and use this to determine if a the maximum CUSUM score of a binary-segmentation operation is significant.
#'
#' Accepts: data(observations), percentile(percentile to find the significance of), simulation.number(how many simulations to use)
#'
#' Returns: critical value for data of that length
#'
...
#' construct.model converts the estimated change-point locations into the estimated signal
#' 
#' Accepts: smapl(observations), ofs(intervals to partition observations)
#' 
...
#' Binary.segmentation.operation will perform one iteration of the binary segmentation algorithm
#' 
#' Accepts: sampl (observations), passed_segments (intervals to be tested), boundary (critical value used in testing)
#'  
#' Returns: finalised_segments (intervals to test in next iteration)
...
#' stored critical values made using bootstrap.of.error. Values exist for observation lengths of 50 - 5000. 
#'
#'
